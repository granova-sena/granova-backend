...
import { buscarPedidoConCliente, listarPedidosPorCliente, contarPedidosPorCliente,insertarPedido } from '../models/pedidosModel.js';
import { buscarDetallePorPedido,insertarDetallePedido } from '../models/detallePedidosModel.js';

export async function obtenerPedidoConDetalle(id_pedido) {
  const { rows } = await buscarPedidoConCliente(id_pedido);
  if (rows.length === 0) {
    throw new ErrorPedido('Pedido no encontrado', 'NO_ENCONTRADO');
  }

  const { rows: detalle } = await buscarDetallePorPedido(id_pedido);
  return { ...rows[0], productos: detalle };
}

export async function obtenerPedidosDeCliente(id_cliente, page, limit) {
  const offset = (page - 1) * limit;

  const [resultado, total] = await Promise.all([
    listarPedidosPorCliente(id_cliente, limit, offset),
    contarPedidosPorCliente(id_cliente),
  ]);
...
export async function crearPedidoCompleto({ id_cliente, metodo_pago, direccion_envio, ciudad_envio, productos, codigo_cupon }) {
