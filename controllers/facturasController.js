...
import { buscarPedidoPorId,obtenerClienteDelPedido } from "../models/pedidosModel.js";
import { obtenerItemsConIva, obtenerProductosDePedido } from "../models/detallePedidosModel.js";
...
export const crearFactura = async (req, res) => {
  const { id_pedido } = req.body;
    ...
    try {
        ...
        if (pedidoEncontrado.estado_pago !== 'pagado') {
            await client.query("ROLLBACK");
            return res.status(400).json({ ok: false, mensaje: "El pedido debe estar pagado para generar la factura" });
        }
        ...
        const esDueno = req.usuario?.id === pedidoEncontrado.id_cliente;
        if (!esAdmin && !esDueno) {
            await client.query("ROLLBACK");
            return res.status(403).json({ ok: false, mensaje: "No tienes permiso para facturar este pedido" });
        }
